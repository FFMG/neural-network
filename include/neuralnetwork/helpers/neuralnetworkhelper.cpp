...
NeuralNetworkHelper::NeuralNetworkHelper(
  ...
  _training_inputs(std::make_shared<const std::vector<std::vector<double>>>(training_inputs)),
  _training_outputs(std::make_shared<const std::vector<std::vector<double>>>(training_outputs)),
  ...
  _training_monitors(std::make_shared<std::vector<TrainingMonitor>>(neural_network.options().has_multi_output()
...
std::vector<std::vector<NeuralNetworkHelperMetrics>> NeuralNetworkHelper::calculate_forecast_metrics(const std::vector<ErrorCalculation::type>& error_types, bool in_sample, bool force_checking_indexes) const
{
  MYODDWEB_PROFILE_FUNCTION("NeuralNetworkHelper");
