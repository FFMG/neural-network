...
private:
  static NeuralNetwork* create_neural_network(Logger::LogLevel log_level)
  {
    ...
    std::vector<LayerDetails> hidden_layers = {
      LayerDetails(Layer::Architecture::FF, 16, activation(activation::method::mish, 1.0), 0.0, 0.01, OptimiserType::NadamW, 0.95, false, 0, 0, 0, 0, 0, 0, 0),
      LayerDetails(Layer::Architecture::FF, 16, activation(activation::method::mish, 1.0), 0.0, 0.01, OptimiserType::NadamW, 0.95, false, 0, 0, 0, 0, 0, 0, 0)
    };
    ...
    auto output_layers = {
      ...
      OutputLayerDetails(1, activation(activation::method::sigmoid, 1.0), ErrorCalculation::type::mse, { 0.0, 0.0, 1.0, 0.0, false, 1.0, 1e-12, 0.0 }, 0.01, OptimiserType::NadamW, 0.95),
      // Second output: Softmax (5-bucket classification)
      OutputLayerDetails(5, activation(activation::method::softmax, 1.0), ErrorCalculation::type::cross_entropy, { 0.0, 0.0, 1.0, 0.0, false, 1.0, 1e-12, 0.0 }, 0.01, OptimiserType::NadamW, 0.95)
    };
