...
- Implemented recurrent-state Layer Normalization for `GRURNNLayer` and `LSTMLayer`, addressing plan item #1 in `can-you-please-write-agile-swan.md` (unstable internal activation scale across BPTT timesteps):
  ...
  - Added a new opt-in `bool use_layer_norm` parameter to `LayerDetails` (`include/neuralnetwork/layers/layerdetails.h`), with a getter and full copy/move/assignment support. Conforming to this codebase's strict no-default-parameters convention, all `LayerDetails(...)` call sites across `examples/`, `tests/`, and `python/examples/` have been explicitly updated to specify `use_layer_norm` (`false` when disabled, `true` when enabled). `Layer::create_hidden_layer` (`include/neuralnetwork/layers/layer.cpp`) now panics if `use_layer_norm` is requested on a non-`Gru`/`Lstm` architecture.
  - `GRURNNLayer` (`include/neuralnetwork/layers/grurnnlayer.h`/`.cpp`) normalizes the blended hidden state `h_t = (1-z)⊙h_{t-1} + z⊙ĥ` in `finalize_forward_step`, in place, before it is cached and propagated — both the value fed downstream and the value carried forward as `h_{t-1}` are normalized. Added `_ln_h_gain_*`/`_ln_h_bias_*` SoA weight/optimiser-state members (values/grads/velocities/m1/m2/timesteps/decays, mirroring the existing z/r gate members), wired through the constructors (fresh-init and deserialization), copy/move/assignment, `calculate_and_store_gradients`/`apply_stored_gradients`/`zero_gradients`/`get_gradient_norm_sq`. `get_pre_activation_multiplier()` now returns a new `LayerNormMultiplier` (`Multiplier + 1`) when `use_layer_norm` is enabled, adding one packed-state slot to cache `inv_std`; disabled layers keep the original `Multiplier` and layout unchanged.
  ...
  - Serialization support in `include/neuralnetwork/helpers/neuralnetworkserializer.cpp`: `add_grurnnlayer`/`add_lstmlayer` save a `use-layer-norm` flag plus the full gain/bias optimiser state when enabled; `create_grurnnlayer`/`create_lstmlayer` load them via `get_or<bool>("use-layer-norm", false)` so models saved before this feature still load; `load_weights` (the partial/values-only reload path) gained matching optional `ln-h-gain-values`/`ln-h-bias-values`/`ln-c-gain-values`/`ln-c-bias-values` handling.
  - Exposed `use_layer_norm` in the Python bindings (`python/bindings.cpp`): `LayerDetails`'s `py::init<...>` gained the trailing parameter (`py::arg("use_layer_norm") = false`) and a matching read-only property.
...
- Added `tests/layer_details_tests.cpp::LayerDetailsUseLayerNormFlag` and `tests/layer_tests.cpp::CreateHiddenLayerRejectsLayerNormOnFFAndElman`/`CreateHiddenLayerAcceptsLayerNormOnGruAndLstm`.
...
- Added `tests/network_integration_tests.cpp::UseLayerNormOptionSerialization`, following the `BpttSuperviseLastStepOnlySerializerSaveLoad` option-survives-round-trip pattern: asserts `use_layer_norm` set via `LayerDetails`/`NeuralNetworkOptions::with_hidden_layers` is still `true` on `loaded_nn->options().hidden_layers()[0]` after a save/load cycle, exercising the `add_hidden_layers`/`get_hidden_layers` fix above.
