...
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
...
RNNLayer::RNNLayer(const RNNLayer& src) noexcept :
...
RNNLayer::RNNLayer(RNNLayer&& src) noexcept :
  _layer_index(src._layer_index),
...
RNNLayer& RNNLayer::operator=(const RNNLayer& src) noexcept
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  if(this != &src)
  {
...
RNNLayer& RNNLayer::operator=(RNNLayer&& src) noexcept
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  if(this != &src)
  {
...
void RNNLayer::move_residual_projector(ResidualProjector* residual_projector)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
...
bool RNNLayer::has_bias() const noexcept
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  return _has_bias_neuron;
}

unsigned RNNLayer::number_neurons() const noexcept
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  return _number_output_neurons;
}

unsigned RNNLayer::number_neurons_with_bias() const noexcept
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
...
RNNLayer RNNLayer::create_input_layer(unsigned num_neurons_in_this_layer, unsigned num_neurons_in_next_layer, double weight_decay)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  return RNNLayer(0, 0, num_neurons_in_this_layer, num_neurons_in_next_layer, weight_decay, -1, LayerType::Input, activation::method::linear, OptimiserType::None, 0.0);
}

RNNLayer RNNLayer::create_hidden_layer(unsigned num_neurons_in_this_layer, unsigned num_neurons_in_next_layer, double weight_decay, const RNNLayer& previous_layer, const activation::method& activation, const OptimiserType& optimiser_type, int residual_layer_number, double dropout_rate)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  return RNNLayer(previous_layer.get_layer_index() + 1, previous_layer._number_output_neurons, num_neurons_in_this_layer, num_neurons_in_next_layer, weight_decay, residual_layer_number, LayerType::Hidden, activation, optimiser_type, dropout_rate);
}

RNNLayer RNNLayer::create_output_layer(unsigned num_neurons_in_this_layer, double weight_decay, const RNNLayer& previous_layer, const activation::method& activation, const OptimiserType& optimiser_type, int residual_layer_number)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  return RNNLayer(previous_layer.get_layer_index()+1, previous_layer._number_output_neurons, num_neurons_in_this_layer, 0, weight_decay, residual_layer_number, LayerType::Output, activation, optimiser_type, 0.0);
}

const std::vector<Neuron>& RNNLayer::get_neurons() const noexcept
{ 
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  return _neurons;
}

std::vector<Neuron>& RNNLayer::get_neurons() noexcept
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  return _neurons;
}

const Neuron& RNNLayer::get_neuron(unsigned index) const 
{ 
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  if (index >= _neurons.size()) 
  {
    Logger::error("Index out of bounds in RNNLayer::get_neuron.");
    throw std::out_of_range("Index out of bounds in RNNLayer::get_neuron.");
  }
  return _neurons[index];
}

Neuron& RNNLayer::get_neuron(unsigned index) 
{ 
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  if (index >= _neurons.size()) 
  {
    Logger::panic("Index out of bounds in RNNLayer::get_neuron.");
  }
  return _neurons[index];
}

std::vector<std::vector<double>> RNNLayer::project_residual_output_values(const std::vector<std::vector<double>>& residual_layer_outputs) const
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
...
std::vector<double> RNNLayer::project_residual_output_values(const std::vector<double>& residual_layer_outputs) const
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
...
WeightParam& RNNLayer::residual_weight_param(unsigned residual_source_index, unsigned target_neuron_index)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  if(nullptr == _residual_projector)
  {
...
const std::vector<std::vector<WeightParam>>& RNNLayer::residual_weight_params() const
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  if(nullptr == _residual_projector)
  {
...
std::vector<std::vector<double>> RNNLayer::calculate_forward_feed(
  const RNNLayer& previous_layer,
  ...
  bool is_training) const
{
  ...
  {
    Logger::panic("RNNLayer::_weights row count != N_prev");
  }
  ...
  for (size_t i = 0; i < N_prev; i++)
  {
    ...
    {
      Logger::panic("RNNLayer::_weights column count != N_this");
    }
  }

  if (!_bias_weights.empty() && _bias_weights.size() != N_this)
  {
    Logger::panic("RNNLayer::_bias_weights size != N_this");
  }
#endif
...
void RNNLayer::calculate_error_deltas(
...
void RNNLayer::calculate_bce_error_deltas(
  std::vector<std::vector<double>>& deltas,
  const std::vector<std::vector<double>>& target_outputs,
...
void RNNLayer::calculate_mse_error_deltas(
  std::vector<std::vector<double>>& deltas,
  const std::vector<std::vector<double>>& target_outputs,
  const std::vector<std::vector<double>>& given_outputs) const
{
...
std::vector<std::vector<double>> RNNLayer::calculate_output_gradients(
  const std::vector<std::vector<double>>& target_outputs,
  const std::vector<std::vector<double>>& given_outputs,
...
std::vector<std::vector<double>> RNNLayer::calculate_hidden_gradients(
  const RNNLayer& next_layer,
...
void RNNLayer::apply_nadam_update(
...
void RNNLayer::apply_nadamw_update(
  WeightParam& weight_param,
  double raw_gradient,
  double learning_rate,
  double beta1,
...
void RNNLayer::apply_adamw_update(
  WeightParam& weight_param,
...
void RNNLayer::apply_adam_update(WeightParam& weight_param, double raw_gradient, double learning_rate, double beta1, double beta2, double epsilon, bool is_bias)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  // Update timestep
  weight_param.increment_timestep();
...
void RNNLayer::apply_none_update(WeightParam& weight_param, double raw_gradient, double learning_rate)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
...
void RNNLayer::apply_sgd_update(WeightParam& weight_param, double raw_gradient, double learning_rate, double momentum, bool is_bias)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
...
void RNNLayer::apply_weight_gradient(const double gradient, const double learning_rate, bool is_bias, WeightParam& weight_param, double clipping_scale, double gradient_clip_threshold)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
  ...
  case OptimiserType::None:
    RNNLayer::apply_none_update(weight_param, clipped_gradient, learning_rate);
    break;

  case OptimiserType::SGD:
    RNNLayer::apply_sgd_update(weight_param, clipped_gradient, learning_rate, _activation.momentum(), is_bias);
    break;

  case OptimiserType::Adam:
    RNNLayer::apply_adam_update(weight_param, clipped_gradient, learning_rate, 0.9, 0.999, 1e-8, is_bias);
    break;

  case OptimiserType::AdamW:
    RNNLayer::apply_adamw_update(weight_param, clipped_gradient, learning_rate, 0.9, 0.999, 1e-8, is_bias);
    break;

  case OptimiserType::Nadam:
    RNNLayer::apply_nadam_update(weight_param, clipped_gradient, learning_rate, 0.9, 0.999, 1e-8);
    break;

  case OptimiserType::NadamW:
    RNNLayer::apply_nadamw_update(weight_param, clipped_gradient, learning_rate, 0.9, 0.999, 1e-8, is_bias);
    break;
...
double RNNLayer::clip_gradient(double gradient, double gradient_clip_threshold)
{
  MYODDWEB_PROFILE_FUNCTION("RNNLayer");
