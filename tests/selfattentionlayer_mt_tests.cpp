...
TEST_F(SelfAttentionLayerMTTest, OddBatchSizeAllGradsThreadCountInvariance) {
    ...
    auto check_grads = [](const std::vector<double>& st, const std::vector<double>& mt, const char* name)
    {
      ASSERT_EQ(st.size(), mt.size()) << name;
      for (size_t i = 0; i < st.size(); ++i)
      {
        EXPECT_NEAR(st[i], mt[i], 1e-12) << name << " index " << i;
      }
    };

    check_grads(layer_st.get_wq_grads(), layer_mt.get_wq_grads(), "wq");
    check_grads(layer_st.get_wk_grads(), layer_mt.get_wk_grads(), "wk");
    check_grads(layer_st.get_wv_grads(), layer_mt.get_wv_grads(), "wv");
    check_grads(layer_st.get_wo_grads(), layer_mt.get_wo_grads(), "wo");
    check_grads(layer_st.get_ff1_w_grads(), layer_mt.get_ff1_w_grads(), "ff1_w");
    check_grads(layer_st.get_ff2_w_grads(), layer_mt.get_ff2_w_grads(), "ff2_w");
    check_grads(layer_st.get_ln1_gain_grads(), layer_mt.get_ln1_gain_grads(), "ln1_gain");
    check_grads(layer_st.get_ln2_gain_grads(), layer_mt.get_ln2_gain_grads(), "ln2_gain");
}
