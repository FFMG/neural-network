...
TEST_F(LSTMLayerTest, ConstructionAndTopology) {
  ...
  EXPECT_EQ(layer.get_pre_activation_multiplier(), 4); // Currently 4
}
...
TEST_F(LSTMLayerTest, ForwardFeedMathematicalVerification) {
  ...
  auto batch_hs = create_batch_hidden_states(topology, 1, 1); // 1 time step
  ...
  // f_pre = 1.0 * 0.2 + 0.0 * 0.3 = 0.2 => f = 1 / (1 + exp(-0.2)) = 0.549834
  // i_pre = 1.0 * 0.4 + 0.0 * 0.5 = 0.4 => i = 1 / (1 + exp(-0.4)) = 0.598688
  // o_pre = 1.0 * 0.6 + 0.0 * 0.7 = 0.6 => o = 1 / (1 + exp(-0.6)) = 0.645656
  // g_pre = 1.0 * 0.5 + 0.0 * 0.1 = 0.5 => g = tanh(0.5) = 0.462117
  
  // c_0 = f * c_{-1} + i * g = 0.549834 * 0 + 0.598688 * 0.462117 = 0.276664
  // h_0 = o * tanh(c_0) = 0.645656 * tanh(0.276664) = 0.645656 * 0.26982 = 0.174211

  const auto rnn_out = batch_go[0].get_rnn_outputs(1);
  EXPECT_NEAR(rnn_out[0], 0.174211, 1e-6);
}
...
TEST_F(LSTMLayerTest, DropoutConsistencyVerification) {
  ...
  // Current implementation will probably NOT be 0.0 because it doesn't reuse the mask
  // Expect it to fail if it's broken.
