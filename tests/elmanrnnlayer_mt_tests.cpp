...
protected:
    ...
    void init_layer_weights(ElmanRNNLayer& layer) 
    {
        ...
        std::vector<double> rw_vals(num_neurons * num_neurons);
...
TEST_F(ElmanRNNLayerMTTest, ForwardFeedMTConsistency) 
{
    ...
    auto batch_hs_st = create_batch_hidden_states(topology, batch_size, num_timesteps, 1);
    
    auto batch_go_mt = create_batch_gradients_and_outputs(topology, batch_size);
    auto batch_hs_mt = create_batch_hidden_states(topology, batch_size, num_timesteps, 1);
...
TEST_F(ElmanRNNLayerMTTest, BackwardFeedMTConsistency) 
{
    const unsigned num_inputs = 8;
    const unsigned num_neurons = 16;
    const unsigned batch_size = 128;
