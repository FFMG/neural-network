...
TEST(SwaTests, LayersAccumulateSwaAveragePropagatesAcrossLayers)
{
  ...
  std::vector<LayerDetails> hidden_layers = {
    LayerDetails(Layer::Architecture::FF, 3, activation(activation::method::linear, 0.0), 0.0, 0.0, OptimiserType::None, 0.0, false, 0, 0, 0, 0, 0, 0, 0)
  };
  auto options = NeuralNetworkOptions::create({ 2, 3, 1 })
    .with_hidden_layers(hidden_layers)
    .with_output_layer_details(OutputLayerDetails(1, activation(activation::method::linear, 0.0), ErrorCalculation::type::mse, { 0.0, 0.0, 1.0, 0.0, false, 1.0, 1e-12, 0.0, { 0.5 }, 0.0, 0.0 }, 0.0, OptimiserType::None, 0.0))
    .build();
