...
TEST_F(ErrorCalculationTest, ComplexArchitectureIntegration) {
  ...
  auto options = NeuralNetworkOptions::create({2, 3, 3, 1})
         ...
         .with_hidden_layers({
           LayerDetails(Layer::Architecture::FF, 3, activation(activation::method::relu, 0.0), 0.0, 0.0, OptimiserType::None, 0.0, false),
           LayerDetails(Layer::Architecture::Elman, 3, activation(activation::method::tanh, 0.0), 0.0, 0.0, OptimiserType::None, 0.0, false)
         })
...
TEST_F(ErrorCalculationTest, MultiOutputArchitectureIntegration) {
  ...
  auto options = NeuralNetworkOptions::create({2, 5, 3}) // 3 outputs
         .with_log_level(Logger::LogLevel::Information)
         .with_hidden_layers({
           LayerDetails(Layer::Architecture::FF, 5, activation(activation::method::relu, 0.0), 0.0, 0.0, OptimiserType::None, 0.0, false)
         })
